...
function simnew(stim,T) #generates new weights and populations with unpotentiated synapses, runs simulation
	...
	return times,ns,popmembers,Ne,Ncells,T,weights_time,weights
end


function sim(stim,weights,popmembers,T) #runs simulation given weight matrix and populations
	...
	#T = 100000 #simulatiogkn time
	Nskip = 10000 #how often (in number of timesteps) to save w_in
	...
	N_steps_save_weights = Nsteps ÷ Nskip
	#weights_time = zeros(Float32,N_steps_save_weights,Ncells,Ncells)
	weights_time = zeros(Float32,size(popmembers,1),N_steps_save_weights)
	w_t = 1
	weights_time[:,w_t] = calc_cluster_weights(popmembers,weights)
	...
	return times,ns,Ne,Ncells,T,weights_time
end
...
function calc_cluster_weights(popmembers,weights_matrix)
    w_in_matrix = zeros(size(popmembers)[1])
	for pop_i = 1:size(popmembers)[1]
		pop_members_indices = findall(popmembers[pop_i,:] .> 0)
		pop_members_values = popmembers[pop_i, pop_members_indices]
		w_in = weights_matrix[pop_members_values,pop_members_values]
		w_in_matrix[pop_i] = mean(w_in[findall(w_in .> 0)])
	end
